...
{
 "use strict";
 var Global,Samples,HelloWorld,SC$1,WebSharper,Operators,List,Obj,UI,Doc,JavaScript,Pervasives,T,Array,Client,Templates,Arrays,JS,View,DomUtility,Unchecked,Object,EventTarget,Node,Collections,Dictionary,AttrProxy,Snap,DocElemNode,CharacterData,Enumerator,SC$2,HashSet,Seq,WindowOrWorkerGlobalScope,SC$3,Docs,T$1,Elt,Attrs,Dyn,DictionaryUtil,Prepare,Slice,KeyCollection,An,Settings,Abbrev,Mailbox,Updates,Attrs$1,Strings,Docs$1,RunState,NodeSet,Anims,SC$4,Fresh,SC$5,SC$6,Concurrency,AppendList,HashSetUtil,Queue,Easing,AsyncBody,SC$7,CT,HashSet$1,SC$8,Scheduler,CancellationTokenSource,DomNodes,Error,OperationCanceledException,Lazy,SC$9,LazyExtensionsProxy,LazyRecord,moment,IntelliFactory,Runtime,console,$,Date;
 Global=self;
 Samples=Global.Samples=Global.Samples||{};
 HelloWorld=Samples.HelloWorld=Samples.HelloWorld||{};
 SC$1=Global.StartupCode$Moment$Client=Global.StartupCode$Moment$Client||{};
 ...
 SC$1.$cctor=function()
 {
  ...
  SC$1.Main=(locales=List.ofArray(["en","fr","es","it","de"]),(zones=List.ofArray([["Cairo","Africa/Cairo"],["New York","America/New_York"],["Tokyo","Asia/Tokyo"]]),(a=Doc.Concat(List.map(function(l)
  {
   var now,diff,ago;
   function m(t,z)
   {
    return Doc.Element("p",[],[Doc.TextNode(t+": "+now.clone().tz(z).format("LLL")),Doc.Element("br",[],[]),Doc.TextNode(ago.from(now)+": "+ago.tz(z).format("LLL"))]);
   }
   now=(new Global.moment()).locale(l);
   diff=(new moment.duration(-4,"hours")).add(-47,"minutes");
   ago=now.clone().add(diff);
   return Doc.Element("div",[],[Doc.Element("h1",[],[Doc.TextNode(l)]),Doc.Element("p",[],[Doc.TextNode("Local "+now.format("LLL")),Doc.Element("br",[],[]),Doc.TextNode(ago.from(now)+": "+ago.format("LLL"))]),Doc.Concat(List.map(function($1)
   {
    return m($1[0],$1[1]);
   },zones))]);
  },locales)),(Templates.LoadLocalTemplates(""),Doc.RunById("main",a)))));
 };
